...
api += board.split('# BEGIN NX733J recovery-only launch properties')[1].split('# END NX733J recovery-only launch properties')[0]
for sdk in [32, 35, 36]:
    fixture = f'PLATFORM_SDK_VERSION := {sdk}\nPRODUCT_SHIPPING_API_LEVEL := 35\n' + api
    fixture += '\n.PHONY: check\ncheck:\n\t@echo "$(BOARD_SHIPPING_API_LEVEL)|$(PRODUCT_SHIPPING_API_LEVEL)|$(PRODUCT_VENDOR_PROPERTIES)|$(TARGET_RECOVERY_ADDITIONAL_PROPERTIES)"\n'
    result = subprocess.check_output(['make', '--no-print-directory', '-f', '-', 'check'], input=fixture, text=True).strip()
    assert result == ('|||ro.product.first_api_level=35 ro.board.first_api_level=35' if sdk == 32 else '35|35||'), result
